  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.token && data.token.accessToken) {
        showModal('Welcome Back!', 'Redirecting you to the dashboard...', 'success');
        
        // Short delay to show the modal
        setTimeout(() => {
          login(data.token.accessToken, data.user);
        }, 1500);
      } else {
        showModal('Login Failed', 'Invalid response from server', 'error');
      }
    } catch (err: any) {
      showModal('Login Failed', err.message || 'Failed to login', 'error');
    } finally {
